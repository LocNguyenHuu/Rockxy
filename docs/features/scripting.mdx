...
Scripts may use either the **multi-arg** API (Proxyman-style, recommended) or the legacy **single-arg** API. Rockxy detects which by inspecting the JS function's `length` at load time, so both work side-by-side.
...
Scripts execute in deterministic, id-sorted order. The first matching script wins on both the request and response side (no chaining in this milestone).
