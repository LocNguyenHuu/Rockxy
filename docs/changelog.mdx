...
<Update label="Unreleased" description="Current branch">
  ### Added

  - Developer Setup Hub: dedicated rows for `Postman`, `Insomnia`, `Paw`, `tvOS / watchOS`, and `Vision Pro`, plus new guide coverage for iOS, Android, Flutter, and React Native targets.
  - Developer Setup Hub: target-specific validation snippets and local proof lanes for Python, Node.js, Ruby, Go, Rust, cURL, Java, Firefox, Docker, and Next.js.

  ### Fixed

  - SSL Proxying controls now stay in sync between the encrypted HTTPS response prompt, sidebar actions, and the main request table.
  - Request table metrics no longer show placeholder request/response byte counts for CONNECT tunnel rows, and tunnel duration now falls back to measured timing when needed.

  ### Changed

  - Inspector tabs now use a single-line scrolling strip, and short tabs such as Query stay top-aligned in both right and bottom layouts.
  - Request table now separates `Status` from `Code`, adds `Request`, `Response`, and `SSL` columns, and updates the SSL icon to reflect effective interception state.
